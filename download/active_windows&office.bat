...
:start
echo 已尝试用管理员运行，如果窗口标题不显示管理员，请手动以管理员运行此脚本
echo. 
echo 适用于VL版本的window系统和office
echo. 
echo 有问题请留言https://tohot.top
echo. 
...
:start2
set KMS_Sev=kms.tohot.top
cls
echo 正在检查激活服务器的连接情况......
echo.
ping kms.tohot.top | find "超时"  > NUL &&  goto fail
ping kms.tohot.top | find "目标主机"  > NUL &&  goto fail
echo 可以连接……
goto office
...
if DEFINED _Office16Path (echo.&echo 已发现 Office2016，请耐心等待激活...
...
if DEFINED _Office%1Path (
    ...
    cscript //nologo ospp.vbs /sethst:kms.tohot.top >nul
...
set KMS_Sev=kms.tohot.top
cls
echo 我的博客：https://tohot.top
echo. 
echo 如果提示未找到序列号，请在CMD里运行下面这行
echo. 
echo reg QUERY "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion" /v "EditionID"
echo. 
echo 把显示结果和系统版本直接在博客评论留言！
echo.
echo.
echo 正在检查激活服务器耐心等待.....
echo.
...
if defined %EditionID% (
	...
	cscript //Nologo %windir%\system32\slmgr.vbs /skms kms.tohot.top
